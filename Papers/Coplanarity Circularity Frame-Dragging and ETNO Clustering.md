...
identical in units to the Newtonian and post-Newtonian accelerations.
...
The symmetric $\varepsilon,\mu$ response, the localized Vacuum Shielding Stress difference, and the velocity-dependent damping term in the three-term equation of motion for massive bodies (and the corresponding term in the optical ray equation for continuous waves) together produce the Lense–Thirring precession rate, drive protoplanetary disks toward relaxed circular coplanar states, and align high-semimajor-axis orbits — all without free parameters beyond angular-momentum conservation and the fixed vacuum response fixed by impedance invariance.
...
In a static potential the secular perturbation is governed by the damping term of the massive-body equation of motion derived in §3.1. The Newtonian and post-Newtonian terms define the reference Keplerian orbit, while the dissipative perturbation is

$$
\mathbf{a}_d=-\frac{\dot n}{n}\mathbf{v},\qquad\dot n=\mathbf{v}\cdot\nabla n.
$$
