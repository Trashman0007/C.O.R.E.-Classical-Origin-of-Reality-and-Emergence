...
A time-varying scalar potential \(\Phi(t,\mathbf{r})\) from an orbiting binary induces VSS strain fluctuations \(\delta u_{\rm vac}\). Electromagnetic/gravity energy spreads over spherical wavefronts; the refractive response therefore couples to the **full transverse 2D plane** (REFORM §3). For a wave propagating in the \(z\)-direction the observer’s wavefront is a 2D disk perpendicular to propagation. Projecting the scalar strain onto the two orthogonal transverse directions yields the two independent transverse-traceless patterns:
\[
h_+ \propto \iint_{\rm disk} \delta u_{\rm vac}(\rho,\varphi)\,\cos(2\varphi)\,\rho\,d\rho\,d\varphi,
\]
\[
h_\times \propto \iint_{\rm disk} \delta u_{\rm vac}(\rho,\varphi)\,\sin(2\varphi)\,\rho\,d\rho\,d\varphi.
\]
These are exactly the plus and cross polarizations observed by LIGO/Virgo. The quadrupolar angular dependence follows from the orbital source motion.

**Propagation speed:** Local \(c\)-invariance (co-scaling of clocks and rulers with \(\varepsilon,\mu\)) forces the measured two-way speed to be exactly \(c\) in every direction, independent of coordinate variations in \(c_{\rm coord}=c/n\). Thus \(v_g=c\) with no measurable dispersion, matching multi-messenger constraints.

**Waveform match:** In the weak-field limit the integrated 2D strain reproduces the standard quadrupole formula and chirp waveform. Existing LIGO/Virgo data (filtered with GR templates) are therefore indistinguishable from C.O.R.E. predictions at current sensitivity.

**Testable differences (future detectors):** Higher-order 2D corrections introduce a small frequency-dependent phase term \(\propto f^2\) (traceable to White et al. dispersion) and possible tiny scalar admixtures detectable by LISA or 3G detectors.
